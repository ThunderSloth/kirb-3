 ...
 *  @brief Embedded bridge dashboard HTML, CSS, and JavaScript.
 */
      ...
      <main class="layout">
        ...
        <div class="stack" id="center-stack">
          ...
          <section class="panel console">
            ...
            <div class="panel-body">
              <pre class="log" id="command-log">[boot] KirbBridge dashboard loaded
[link] Waiting for MSP telemetry
  ...
  <script>
    ...
    let consolePollPending = false;
    let commandPending = false;
    let commandPendingTimer = null;
    let latestTelemetry = null;
    let ultDisplayUnit = "cm";
    const telemetryPollMs = 100;
    const consolePollMs = 300;
    ...
    function setCommandPendingState(pending) {
      commandPending = pending;
      commandInput.disabled = pending;
      commandButton.disabled = pending;

      if (!pending && commandPendingTimer !== null) {
    ...
    function setTelemetryStampState(fresh) {
      const color = fresh ? "#9fe870" : "#93a0ad";
      telemetryStamp.style.color = color;
      telemetryStamp.style.setProperty("--pill-color", color);
      telemetryStamp.style.setProperty(
        "--pill-ring",
        fresh ? "rgba(159, 232, 112, 0.12)" : "rgba(147, 160, 173, 0.10)"
      );
      telemetryStamp.style.setProperty(
        "--pill-pulse",
        fresh ? "pulse 1.8s infinite" : "none"
      );
    }
    ...
    function formatUltrasonicValue(pwUs) {
      ...
      switch (ultDisplayUnit) {
        ...
        case "pw":
          return {
            valueText: Math.round(pwUs).toString(),
            barValue: pwUs,
            barMin: 0,
            barMax: 20000
          };
    ...
    function colorForBattery(volts) {
      const clamped = Math.max(10.8, Math.min(12.6, volts));
      const normalized = (clamped - 10.8) / (12.6 - 10.8);
      const hue = normalized * 120.0;
      const hueLo = Math.max(0, hue - 8.0);
      const hueHi = Math.min(120, hue + 8.0);
      return "linear-gradient(90deg, hsl(" + hueLo.toFixed(1) + " 62% 38%), hsl(" +
        hueHi.toFixed(1) + " 78% 64%))";
    }
    ...
    async function pollConsole() {
      ...
      try {
        ...
        if (consolePollPending) {
          consolePollPending = false;
          window.setTimeout(pollConsole, 0);
        }
      }
    }
    ...
    async function loadCameraStreamUrl() {
      try {
        const response = await fetch("/camera_url", { cache: "no-store" });
        if (!response.ok) {
          throw new Error("HTTP " + response.status);
        }
    ...
    commandForm.addEventListener("submit", async function(event) {
      ...
      try {
        ...
        if (result.accepted) {
          pollConsole();
          window.setTimeout(pollConsole, 80);
          commandPendingTimer = window.setTimeout(function() {
            setCommandPendingState(false);
          }, 1500);
        } else {
          setCommandPendingState(false);
    ...
    setTelemetryStampState(false);
    loadCameraStreamUrl().then(function() {
      connectStream();
      pollTelemetry();
      pollConsole();
