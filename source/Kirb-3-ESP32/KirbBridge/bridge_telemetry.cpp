...
constexpr int kTelemetryRxPin = 4;
constexpr int kTelemetryTxPin = 5;
...
bool parseTelemetryLine(char *line, TelemetrySnapshot &out)
{
    ...
    while ((token = strtok_r(nullptr, ",", &context)) != nullptr)
    {
        ...
        else if (strcmp(token, "ENC_OK") == 0)
        {
            ...
            sawEncOk =
                parseUint16Token(strtok_r(nullptr, ",", &context), encOk0);
            if (!sawEncOk)
            {
                return false;
            }
            ...
            else if (nextToken != nullptr && strcmp(nextToken, "ENC") == 0)
            {
                ...
                sawEnc =
                    parseInt32Token(strtok_r(nullptr, ",", &context), out.enc[0]);
                if (!sawEnc)
                {
                    return false;
                }
                sawEnc =
                    parseInt32Token(strtok_r(nullptr, ",", &context), out.enc[1]);
                if (!sawEnc)
                {
                    return false;
                }
    ...
    if (!(sawRc && sawUlt && sawUltEn && sawMtr && sawSrv && sawBat &&
          sawImuOk && sawImu && sawEncOk && sawEnc))
    {
        return false;
    }
...
void handleCompleteLine()
{
    ...
    if (strncmp(gLineBuffer, "ACK,", 4) == 0 ||
        strncmp(gLineBuffer, "ERR,", 4) == 0 ||
        strncmp(gLineBuffer, "LOG,", 4) == 0 ||
...
{
    kTelemetrySerial.begin(kTelemetryBaud, SERIAL_8N1, kTelemetryRxPin,
                           kTelemetryTxPin);
    gLineLength = 0;
}
...
size_t bridgeTelemetryBuildJson(char *buffer, size_t bufferLen)
{
    ...
    const bool fresh = snapshot.hasData &&
                       (millis() - snapshot.lastUpdateMs) <= kTelemetryFreshMs;
    ...
    return static_cast<size_t>(snprintf(
        ...
        fresh ? "true" : "false",
        static_cast<unsigned long>(snapshot.lastUpdateMs), snapshot.rc[0],
        snapshot.rc[1], snapshot.rc[2], snapshot.rc[3], snapshot.rc[4],
        snapshot.rc[5], snapshot.ult[0], snapshot.ult[1], snapshot.ult[2],
        snapshot.ult[3], snapshot.ult[4], snapshot.ult[5], snapshot.ult[6],
        snapshot.ult[7], snapshot.ult[8], snapshot.ult[9], snapshot.ultEn[0],
        snapshot.ultEn[1], snapshot.ultEn[2], snapshot.ultEn[3],
        snapshot.ultEn[4], snapshot.ultEn[5], snapshot.ultEn[6],
        snapshot.ultEn[7], snapshot.ultEn[8], snapshot.ultEn[9],
        snapshot.mtr[0], snapshot.mtr[1], snapshot.srv[0], snapshot.srv[1],
        snapshot.batMv, snapshot.imuOk ? "true" : "false",
        static_cast<int>(snapshot.imuAccelMg[0]),
        static_cast<int>(snapshot.imuAccelMg[1]),
        static_cast<int>(snapshot.imuAccelMg[2]),
        static_cast<int>(snapshot.imuGyroMdps[0]),
        static_cast<int>(snapshot.imuGyroMdps[1]),
        static_cast<int>(snapshot.imuGyroMdps[2]),
        snapshot.encValid[0] ? "true" : "false",
        snapshot.encValid[1] ? "true" : "false",
        static_cast<long>(snapshot.enc[0]),
        static_cast<long>(snapshot.enc[1])));
}
...
bool bridgeTelemetryQueueCommand(const char *command, char *response,
                                 size_t responseLen)
{
...
size_t bridgeTelemetryBuildConsoleJson(uint32_t sinceSeq, char *buffer,
                                       size_t bufferLen)
{
    uint32_t nextSeq = 0;
    ...
    latestSeq = (nextSeq == 0u) ? 0u : (nextSeq - 1u);

    size_t used = 0;
    int written = snprintf(buffer, bufferLen, "{\"next\":%lu,\"lines\":[",
                           static_cast<unsigned long>(latestSeq));
    if (written < 0)
    {
        return 0;
    }
